...
document.getElementById("education").innerHTML = `
    ...
    ${cvData.education.map(function(education) {
        return `
        ...
        ${education.details ? `<span class="education--details col-12">${education.details}</span>` : ""}
        </div>`
    }).join("")}
...
document.getElementById("publications").innerHTML = `
    ...
    <h4 class="subtitle row">Philosophy</h4>
        ...
        ${cvData.publications.filter(item => item.type == "empirical").map(function(publication) {
            ...
            <a href=${publication.link} target="_blank">
            <h6 class="publication--title col-12">'${publication.title}'</h6></a> 
            <span class="publication--authors col-12">Authors: ${publication.authors.replace(regex, "<u>$1</u>")}</span>
            <span class="publication--info col-12">(${publication.date}). ${publication.conference ? publication.info : publication.info.italics()}.</span>
            </li>
        `
        }).join("")}
...
document.getElementById("raships").innerHTML = `
    ...
    ${cvData.raship.map(function(raship) {
        return `
...
document.getElementById("presentations").innerHTML = `
    ...
    <ol>
    ${cvData.presentations.map(function(presentation) {
        return `
                <li>
                <div class="row">
                    <h6 class="col-12">${presentation.title} ${presentation.peerReviewed ? "<span class='small'>* Peer Reviewed</span>" : ""}</h6>
...
document.getElementById("teaching").innerHTML = `
    ...
    <h4 class="subtitle row">Assistant Instructor</h4>
        ${cvData.teaching.ai.map(function(teaching){
            return `
    ...
    ${cvData.teaching.ta.map(function(teaching){
        return `
            <div class="teaching row">
            <span class="col-7">${teaching.title}</span>
    ...
    ${cvData.teaching.other.map(function(teaching){
        return `
            <div class="teaching row">
            <span class="col-7">${teaching.title}</span>
            <span class="col-5">${teaching.semester}</span>
...
document.getElementById("service").innerHTML = `
    ...
    ${cvData.service.department.map(function(service) {
        return `
    ...
    <div class="service row">
    <span>Reviewer for ${cvData.service.profession.reviews.map(function(service) {
        return `
...
document.getElementById("coursework").innerHTML = `
    ...
    ${cvData.coursework.map(function(course) {
        return `
...
document.getElementById("skills").innerHTML = `
    ...
    ${cvData.skills.map(function(skill) {
        return `
